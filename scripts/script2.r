...
df <-  
  expand.grid( 
    ...
    # Incorporating doParallel for the execution of MTweedieTests within the loop
    library(doParallel)
    library(foreach)
    num_cores <- detectCores() - 1  # saving one core for the OS
    registerDoParallel(cores=num_cores)
    
    # Modify the execution of MTweedieTests to use parallel processing
    ...
    # Stopping the cluster after the operations
    stopImplicitCluster()
