...
MTweedieTests <-  
  function(N,M,sig){ 
    ...
    # Correcting parallel computing within the MTweedieTests function
    library(doParallel)
    num_cores <- detectCores() - 1  # Saving one core for other operations
    cl <- makeCluster(num_cores)
    registerDoParallel(cl)  # Register the parallel backend with the cluster
    
    # Running the simulations in parallel, making sure to export the 'simTweedieTest' function
    results <- foreach(x = 1:M, .export = c('simTweedieTest'), .packages = 'tweedie', .combine = 'c') %dopar% {
      simTweedieTest(N) < sig
    }
    stopCluster(cl)  # Stopping the cluster after the operations are done
    sum(results)/M  # Calculating the final result
